...
/// A fresh cookie-jar client — the real `engine::build_client` (cookie-only auth, no header
/// injection, timeouts + redirect policy), so live runs exercise the shipped client exactly.
fn new_client() -> Client {
    let (client, _jar) = crate::engine::build_client("").expect("reqwest client");
    client
}
...
async fn authed(base: &str, user: &str, pass: &str) -> (Client, Endpoints, String) {
    ...
    assert!(
        login::verify_session(&client, &ep).await,
        "verify_session=false after login"
    );
...
async fn live_login_teacher() {
    let (_, _, user_no) = authed(
        &env("TRON_BASE_URL"),
        &env("TRON_TEACHER_USER"),
        &env("TRON_TEACHER_PASS"),
    )
    .await;
...
async fn live_llm() {
    ...
    let school = new_client();
    let llm_client = llm::build_client().expect("llm client");
    let messages =
        vec![serde_json::json!({"role":"user","content":"單選題：2+2=? A) 3  B) 4  C) 5  D) 6"})];
    let ans = llm::answer_question(
        &llm_client,
        &school,
        &cfg,
        &messages,
    ...
    assert!(
        ans.to_uppercase().contains('B'),
        "expected the letter B, got {ans:?}"
    );
}
...
async fn live_school_probe() {
        ...
        set.spawn(async move {
            ...
            let (label, known) = match client.get(&url).send().await {
                Ok(r) => {
                    ...
                    (
                        format!("HTTP {} → {kind}", status.as_u16()),
                        status.is_success() && kind != "Unknown",
                    )
                }
    ...
    assert!(
        known * 100 / total >= 60,
        "too many schools unreachable/unclassified: {known}/{total}"
    );
}
...
fn live_monitor_pipeline() {
    ...
    let h = crate::core_init(Some(mon_collect));
    ...
    assert!(
        signed.is_some(),
        "monitor did not detect+sign the seeded rollcall end-to-end"
    );
}
...
async fn live_exam_answer() {
    ...
    let paper = answer::fetch_paper(&client, &ep, Source::Exam, &exam, "")
        .await
        .expect("fetch_paper");
    println!(
        "exam subjects={} instance_id={:?}",
        paper.subjects.len(),
        paper.instance_id
    );
    assert!(
        !paper.subjects.is_empty(),
        "no subjects — exam not open / not distributed?"
    );
    ...
    let llm_client = llm::build_client().expect("llm client");
    let answers = answer::shared_answers(
        &client,
        &llm_client,
        &cfg,
        noop_cb as EventCb,
    ...
    println!(
        "answered {}/{} subjects; missing={missing:?}",
        answers.len(),
        paper.subjects.len()
    );
    assert!(
        missing.is_empty(),
        "LLM left subjects unanswered: {missing:?}"
    );

    let (sid, retake) = answer::submit_exam(
        &client,
        &ep,
        &exam,
        &paper.instance_id,
...
async fn live_number_rollcall() {
    ...
    let signed = rollcall::sign_number(
        &client,
        &ep,
        &id,
        &user_no,
    ...
    assert!(
        present,
        "student not on_call_fine after number sign (sign result: {signed:?})"
    );
}
...
async fn live_radar_rollcall() {
    ...
    let signed =
        rollcall::sign_radar(&client, &ep, &id, &strategies, &user_no, "v2-live-test").await;
    println!("sign_radar = {signed:?}");
    let present = rollcall::recheck_on_call_fine(&client, &ep, &id, &user_no).await;
    println!("recheck_on_call_fine = {present}");
    assert!(
        present,
        "student not on_call_fine after radar sign (sign result: {signed:?})"
    );
}
...
async fn live_exam_submit_contract() {
    ...
    let paper = answer::fetch_paper(&client, &ep, Source::Exam, &exam, "")
        ...
        .expect("fetch_paper");
    assert!(
        !paper.instance_id.is_empty(),
        "instance_id empty (int extraction regressed)"
    );
    ...
    for s in &paper.subjects {
            ...
            .and_then(|o| o.first())
        {
            ...
            if !oid.is_empty() {
                answers.insert(
                    crate::quiz::subject_id(s),
                    crate::quiz::Answer::Options(vec![oid]),
                );
            }
        }
    }
    println!(
        "guess answers for {}/{} subjects; instance_id={}",
        answers.len(),
        paper.subjects.len(),
        paper.instance_id
    );
    ...
    answer::resubmit_correct(&client, &ep, &exam, &sid)
        .await
        .expect("resubmit_correct");
    // Poll the best score (grading can lag the resubmit by a beat).
    let mut score = None;
    for _ in 0..6 {
        let list: serde_json::Value = client
            .get(ep.exam_submissions(&exam))
            .send()
            .await
            .unwrap()
            .json()
            .await
            .unwrap();
        score = list.get("exam_score").and_then(|v| {
            v.as_f64()
    ...
    assert_eq!(
        score,
        Some(60.0),
        "resubmit_correct did not reach full marks — leak-replay id-type?"
    );
}
...
async fn live_course_context() {
    ...
    let course = env("TRON_COURSE");
    let text = crate::course_context::search_course_materials(
        &client,
        &base,
        &course,
        "講義 material pdf",
