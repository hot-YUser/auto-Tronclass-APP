...
fn boot(tag: &str) -> (Harness, String, String) {
    ...
    let dir = std::env::temp_dir()
        .join(format!("tron-r2-{tag}-{}", new_id()))
        .to_string_lossy()
        .replace('\\', "/");
    let i = hz.next();
    send(
...
fn radar_solver_locates_and_signs() {
    ...
    post(
        &base,
        "/_test/open_rollcall",
        r#"{"id":"RAD","kind":"radar","lat":35.6812,"lng":139.7671,"requires_coords":true,"scope_radius":100}"#,
    );
    assert!(
        wait_for(signed("RAD", &dave), 30).is_some(),
        "solver reverse-locates the hidden target and signs"
    );
    drop(hz);
}
...
fn radar_beacon_attaches_signal() {
    ...
    post(
        &base,
        "/_test/open_rollcall",
        r#"{"id":"RADB","kind":"radar","lat":-33.8568,"lng":151.2153,"requires_coords":true,"scope_radius":100,"use_beacon":true,"beacon_nonce":"nonce-xyz"}"#,
    );
    assert!(
        wait_for(signed("RADB", &dave), 30).is_some(),
        "beacon radar signs"
    );
    let saw: Value =
        serde_json::from_str(&get(&base, "/_test/saw_radar_signal")).unwrap_or(Value::Null);
    assert_eq!(
        saw["saw"], true,
        "coordinate answers carried a radarSignal when use_beacon"
    );
    drop(hz);
}
