...
fn boot(tag: &str, base: &str, budget: u64, reask: u32) -> (Harness, String) {
    ...
    let dir = std::env::temp_dir()
        .join(format!("tron-r4-{tag}-{}", new_id()))
        .to_string_lossy()
        .replace('\\', "/");
    let i = hz.next();
    send(
...
fn per_family_detection_and_gates() {
    ...
    post(
        &base,
        "/_test/open_quiz",
        &format!(r#"{{"activity_id":"EX_OK","course_id":"C1","source":"exam",{leaked}}}"#),
    );
    post(
        &base,
        "/_test/open_quiz",
        &format!(
            r#"{{"activity_id":"CL_OK","course_id":"C1","source":"classroom-exam","started_subjects_count":1,{leaked}}}"#
        ),
    );
    ...
    post(
        ...
        "/_test/open_quiz",
        &format!(
            r#"{{"activity_id":"CW_OK","course_id":"C1","source":"courseware-quiz","my_submission":false,{leaked}}}"#
        ),
    );
    ...
    post(
        ...
        "/_test/open_quiz",
        &format!(
            r#"{{"activity_id":"CL_ZERO","course_id":"C1","source":"classroom-exam","started_subjects_count":0,{leaked}}}"#
        ),
    );
    post(
        &base,
        "/_test/open_quiz",
        &format!(
            r#"{{"activity_id":"EX_DONE","course_id":"C1","source":"exam","has_submitted":true,{leaked}}}"#
        ),
    );
    post(
        &base,
        "/_test/open_quiz",
        &format!(
            r#"{{"activity_id":"EX_EXHAUST","course_id":"C1","source":"exam","submit_times":1,"submission_count":1,{leaked}}}"#
        ),
    );
    post(
        &base,
        "/_test/open_quiz",
        &format!(
            r#"{{"activity_id":"CW_DONE","course_id":"C1","source":"courseware-quiz","my_submission":true,{leaked}}}"#
        ),
    );
    post(
        &base,
        "/_test/open_quiz",
        r#"{"activity_id":"VT_VOTED","course_id":"C1","source":"vote","vote_type":"single","vote_items":{"A":"a","B":"b"},"vote_students":[{"user_no":"dave"}]}"#,
    );
    post(
        &base,
        "/_test/open_quiz",
        r#"{"activity_id":"EMPTY1","course_id":"C1","source":"exam","subjects":[]}"#,
    );
    ...
    post(
        ...
        "/_test/open_quiz",
        &format!(
            r#"{{"activity_id":"EX_EXPIRED","course_id":"C1","source":"exam","is_closed":false,"end_time":"2000-01-01T00:00:00Z",{leaked}}}"#
        ),
    );

    assert!(
        wait_for(submitted("EX_OK"), 25).is_some(),
        "control exam submits"
    );
    assert!(
        wait_for(submitted("CL_OK"), 25).is_some(),
        "control classroom submits"
    );
    assert!(
        wait_for(submitted("CW_OK"), 25).is_some(),
        "control courseware (material→quizzes chain) submits"
    );
    for gated in [
        "CL_ZERO",
        "EX_DONE",
        ...
        assert!(
            !any(submitted(gated)),
            "{gated} must be gated out (never submitted)"
        );
    }
    drop(hz);
}
...
fn expiry_mode_recovers(tag: &str, mode: &str) {
    ...
    post(
        &base,
        "/_test/expire",
        &format!(r#"{{"expired":true,"mode":"{mode}"}}"#),
    );
    assert!(
        wait_for(
            |v| crate::test_support::event_account_login_state(v, &dave, "online"),
...
fn r3c_retry_then_whole_paper_submits() {
    ...
    post(
        &base,
        "/_test/open_quiz",
        r#"{"activity_id":"RT1","course_id":"C1","source":"exam","subjects":[{"id":"s1","type":"short_answer","content":"why"}]}"#,
    );
    assert!(
        wait_for(submitted("RT1"), 30).is_some(),
        "re-prepare fills the subject → whole paper submits"
    );
    drop(hz);
}
...
fn sign_time_expiry_recovers(tag: &str, open_json: &str, rc: &str) {
    ...
    assert!(
        wait_for(signed_in(rc), 30).is_some(),
        "{rc}: re-signs to SignedIn after a sign-time expiry"
    );
    drop(hz);
}
#[test]
fn sign_time_expiry_number_recovers() {
    let _g = SEQ.lock().unwrap();
    sign_time_expiry_recovers(
        "sx-num",
        r#"{"id":"RC_NUM","kind":"number","number_code":"1234","attendance_rate":100.0}"#,
        "RC_NUM",
    );
}
#[test]
fn sign_time_expiry_radar_recovers() {
    ...
    sign_time_expiry_recovers(
        "sx-rad",
        r#"{"id":"RC_RAD","kind":"radar","requires_coords":false,"attendance_rate":100.0}"#,
        "RC_RAD",
    );
}
#[test]
fn sign_time_expiry_self_reg_recovers() {
    ...
    sign_time_expiry_recovers(
        "sx-self",
        r#"{"id":"RC_SELF","kind":"self_registration","attendance_rate":100.0}"#,
        "RC_SELF",
    );
}
...
fn double_sign_guard_only_reauthed_account_resigns() {
    ...
    post(
        &base,
        "/_test/open_rollcall",
        r#"{"id":"RC_DS","kind":"self_registration","attendance_rate":100.0}"#,
    );
    let sib = |acc: &str, ev: &Value| {
        ev["event"] == "SignedIn"
            && ev["rollcall_id"] == "RC_DS"
            && ev["account_id"].as_str() == Some(acc)
    };
    assert!(
        wait_for(|v| sib(&b, v), 30).is_some(),
        "B recovers + signs after its re-login"
    );
    assert!(wait_for(|v| sib(&a, v), 30).is_some(), "A signs");
    let a_signins = snapshot().iter().filter(|v| sib(&a, v)).count();
    assert_eq!(
        a_signins, 1,
        "A must sign EXACTLY once — never re-signed by B's recovery"
    );
    drop(hz);
}
...
fn r3c_persistent_unanswerable_errors_no_submit() {
    ...
    post(
        &base,
        "/_test/open_quiz",
        r#"{"activity_id":"PF1","course_id":"C1","source":"exam","subjects":[{"id":"s1","type":"short_answer","content":"why"}]}"#,
    );
    assert!(
        wait_for(
            |v| v["event"] == "Error" && v["code"] == "quiz_unanswerable",
            25
        )
        .is_some(),
    ...
    assert!(
        !any(submitted("PF1")),
        "a persistently-unanswerable paper is never submitted (no half-paper)"
    );
    drop(hz);
}
