...
**實作（slice 4）**：已有 `KeyStore` trait（`store`/`load`/`available`）＋ in-memory `MemKeyStore` stub；
`VaultFile::unlock_with_key` 可用平台存的那把 key 免密碼解鎖（`UnlockWithKeystore` 命令），主密碼路徑不變。
`Caps.biometric_unlock` 由 `KeyStore::available()` 驅動（stub＝false）。真 Keychain/Keystore/DPAPI → Phase B（要真裝置）。

## 設定（結構化，不是文字檔）

設定是**結構化資料 + 一個 Settings UI**，不是手改的文字檔。core 開 typed config get/set；UI 畫設定頁。
（手機上沒有記事本可改檔，桌面也不該逼使用者編輯 config 文字。）
**實作（slice 4）**：所有旋鈕收進 typed `Settings`（含 `operating` 排程 + `tz_offset_minutes`），經
`UpdateConfig` patch、落 `config.json`。排程 gate＝純函式 `Operating::is_open(epoch, tz)`；monitor 的 poller
只在「今天星期幾的 enabled 窗內」才輪詢（空排程＝全時段開）。本地時間＝UTC＋固定 offset（不處理 DST，零依賴）。
...
## 目標架構矩陣（native core 的 ABI/RID 覆蓋）

Rust core 對每個平台都出**同一份**領域邏輯；沒有平台專屬功能。目標覆蓋：
