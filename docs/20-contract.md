...
SetAnswer { quiz_id, account_id, subject_id, answer }   // 手動改某題（衝突定案；account_id 因衝突/答案 per-account）
...
NextClass { account_id, course, start_time, location }  // 首頁「下一堂課」；不發＝沒有下一堂→整塊隱藏。
                                                     // ⚠ core 尚未發送（TODO：由 /api/my-courses 推導）；MockCore 已給樣本可先做
