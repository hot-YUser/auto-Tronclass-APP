...
- **首頁**：監控總開關(`StartMonitoring`/`StopMonitoring`)、被監控帳號摘要(`AccountStatus`)、目前狀態
  (`StateChanged.state`)、下一堂課、近期活動、背景可用性(`Caps.background_monitoring`；iOS 顯示「僅前景」)。
...
- 建置：`pwsh ./build-core.ps1` 產生原生庫，再 `dotnet build ui/Ui.csproj -f net11.0-windows10.0.19041.0`
  （見 `README.md`）。開發預覽用 `MockCore`，不需要原生庫也能跑 UI 邏輯。
