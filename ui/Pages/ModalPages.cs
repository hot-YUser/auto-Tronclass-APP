...
/// <summary>保險庫閘門:exists=false 建立、unlocked=false 解鎖;解鎖成功由 coordinator 收掉。</summary>
public sealed class VaultModalPage : ModalPageBase
{
    readonly AppState _state;
    readonly Label _title = Theme.Title("");
    readonly Label _hint = Theme.Dim("", 13);
    readonly Entry _pw = new() { IsPassword = true, Placeholder = "主密碼" };
    readonly Entry _pw2 = new() { IsPassword = true, Placeholder = "再次輸入主密碼" };
    readonly Label _error;
    readonly Button _primary;

    public VaultModalPage(AppState state)
    {
        _state = state;
        _error = Theme.Text("", 12.5, Theme.FontRegular, Theme.DangerL, Theme.DangerD);
        _error.IsVisible = false;

        _primary = Theme.Primary("", Submit);
        var biometric = Theme.Ghost("生物辨識解鎖", state.UnlockWithKeystore);
        biometric.BindingContext = state.Caps;
        biometric.SetBinding(IsVisibleProperty, nameof(CapsVm.BiometricUnlock));

        _pw.Completed += async (_, _) => await Submit();
        _pw2.Completed += async (_, _) => await Submit();

        SetCard(new VerticalStackLayout
        {
            Spacing = 12,
            Children = { _title, _hint, _pw, _pw2, _error, _primary, biometric },
        });

        state.VaultStateChanged += Render;
        Render();
    }

    bool CreateMode => !_state.VaultExists;

    void Render()
    {
        _title.Text = CreateMode ? "建立保險庫" : "解鎖保險庫";
        _hint.Text = CreateMode
            ? "帳號與金鑰以主密碼加密存放在本機。請設定一組主密碼。"
            : "輸入主密碼以繼續。";
        _pw2.IsVisible = CreateMode;
        _primary.Text = CreateMode ? "建立" : "解鎖";
        _error.IsVisible = false;
    }
    ...
    {
        var pw = _pw.Text ?? "";
        if (pw.Length == 0) { ShowError("請輸入主密碼"); return; }
        if (CreateMode && pw != (_pw2.Text ?? "")) { ShowError("兩次輸入的密碼不一致"); return; }
        _error.IsVisible = false;
        await (CreateMode ? _state.CreateVault(pw) : _state.Unlock(pw));
        _pw.Text = _pw2.Text = "";
    }

    void ShowError(string msg) { _error.Text = msg; _error.IsVisible = true; }

    // 鎖定畫面刻意擋 Android 硬體返回:不可繞過解鎖進 App(明確覆寫,消除「無意間繼承」的疑慮)。
    // 使用者仍可用系統 Home 手勢背景化;不寫平台碼去自訂 minimize(違反零平台判斷)。
    protected override bool OnBackButtonPressed() => true;
}
